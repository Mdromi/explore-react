...
class PollForm extends Component {
  ...
  createOption = () => {
    ...
    if (options.length < 5) {
      ...
      this.setState({ options });
    } else alert("We can create max 5 options");
  };
  ...
  deleteOption = (index) => {
    ...
    if (options.length > 2) {
      ...
      this.setState({ options });
    } else alert(`You must at least to options`);
  };
  ...
  validate = () => {
    ...
    else if (title.length < 20) errors.title = "Title Too Short";
    else if (title.length > 100) errors.title = "Title Too Long";
    ...
    options.forEach((opt, index) => {
      if (!opt.value) optionErrors[index] = "Option Text Empty";
